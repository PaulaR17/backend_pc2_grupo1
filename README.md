# Backend PC2 - Grupo 1

This repository contains the backend for the PC2 project.

It is built using Laravel and will be used to implement the API and manage the database of the application.

At this stage, the project includes:

- Laravel base structure
- PostgreSQL connection configured
- Initial environment setup

The database runs in Docker on the virtual machine and is accessed from the local development environment.

The `.env` file is not included in the repository.
